...
| Save a 20 MB `.xarast`, container part (no SVG serialisation yet) | ≤ 1 s | **92 ms** in memory, 102 ms through `write_atomic` to disk | passes (XARA-US-0021, 2026-09-23); table in "`.xarast` container" |
| Re-save a 300 MB photo `.xarast` with unchanged resources | ≤ 1 s | **101 ms** | passes: raw copies, no rehash, no recompression |
| Open a 20 MB `.xarast`: signature + manifest + thumbnail | ≤ 15 ms | **0.048 ms** | passes |
| BLAKE3 throughput, one core | ≥ 1 GB/s | **5.4 GiB/s** | passes |
...
`taskset -c 4-7`, 2026-09-23 at load ~4 (two fuzzers running). Criterion
medians; the spread across samples is under 0.3 %.
...
|---|---|---|---|
| `save_20mb` | `PackageWriter` → `Vec`: BLAKE3 of meta/document/thumbnail, compression policy, DEFLATE level 6 (zlib-rs), manifest, ZIP | **92.1 ms** | 217 MiB/s of input |
| `save_20mb_atomic` | the same through `write_atomic` to a file on the local disk: temporary, fsync, rename, directory fsync | **102.4 ms** | 195 MiB/s |
| `open_20mb` | `XarastReader::open` (sniff, end record, central directory, manifest parse, consistency) + `thumbnail()` | **47.9 µs** | — |
| `resave_300mb_raw` | open a 310 MB package (30 × 10 MiB resources), `from_package`, `carry_from`, `finish_with_source` into a `Vec` | **100.9 ms** | 1.55 GiB/s: a `memcpy` of compressed bytes |
| `blake3_64mb` | `Digest::of`, single-threaded | **11.5 ms** | 5.43 GiB/s |
...
rest is DEFLATE of the SVG. If the SVG serialiser needs the headroom,
deflating the document in parallel chunks is the lever; nothing else in
the container looks worth optimising.
