...
45. **The text tool (XARA-US-0047)** is a state machine over two states
...
46. **The tool lays stories out itself**, through the walker's bridge
...
47. **Text is picked by its line box.** The pick index adds each
...
48. **Navigation keys are an intent, not `ToolAction`s**: `Intent::TextNav(
...
9. The shape editor never changes a quick shape's kind except through
   `ConvertToPaths`.
10. The text tool never mutates the document: entering, selecting,
    navigating and preparing a new story leave the canonical digest and
    the undo history as they were (`tests/text_tool.rs`).
11. While a text caret is up, Delete/Backspace/Enter never reach the
